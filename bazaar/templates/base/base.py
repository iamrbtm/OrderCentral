...
def home():
    ...
    interests = db.session.query(Booking).filter(Booking.active == True,
                                                 and_(Booking.cl_appsubmission == False,
                                                      Booking.cl_appapproved == False)).order_by(
        Booking.info_datestart).all()
